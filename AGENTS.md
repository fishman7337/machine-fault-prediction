...
- **Project:** Predictive Maintenance Fault Classifier
...
- Install the full notebook/runtime stack with `python -m pip install -r requirements-all.txt` only when notebook or full training work requires it.

## Managed artifacts and data

- Treat `data/raw/factory_data.csv` as the canonical committed input; generated processed data, models, metrics, and figures remain outside Git.
- Load only trusted serialized model artifacts and rerun real loading or inference checks after dependency or serialization changes.

## Integration and claim boundaries

- Python 3.12 currently emits a non-failing upstream scikit-learn/SciPy deprecation warning.
- Results are dataset-specific and do not establish industrial reliability or production accuracy.
- **Verified local capability:** Python 3.11/3.12; six-estimator pipeline.
- **Known boundary:** Python 3.12 emits an upstream scikit-learn/SciPy deprecation warning.
